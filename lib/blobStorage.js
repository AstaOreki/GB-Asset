...
// News posters are public marketing images (not sensitive like a receipt),
// so unlike the store above they use a SEPARATE Blob store created in
// public mode — access: "public" against the private receipts store would
// throw, the same way "private" would throw against a public one. Its
// token is passed explicitly (rather than relying on the ambient
// BLOB_READ_WRITE_TOKEN, which points at the private receipts store) so
// this never accidentally writes into the wrong store. The result is a
// plain CDN URL any browser can load directly in an <img>, no proxy route
// needed.
export async function uploadNewsPosterBlob(objectPath, buffer, contentType) {
  const { url } = await put(objectPath, buffer, {
    access: "public",
    addRandomSuffix: true,
    contentType,
    token: process.env.NEWS_BLOB_READ_WRITE_TOKEN,
  });
  return url;
}
